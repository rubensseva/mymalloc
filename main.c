...
void mymalloc_init() { 
  ...
  m->size = (MEM_SIZE - sizeof(struct mem_control_block)) / sizeof(struct mem_control_block);
...
void *mymalloc(long _numbytes) {
  ...
  long numunits = (_numbytes + sizeof(struct mem_control_block) - 1)/sizeof(struct mem_control_block);
  printf("Allocating %lu bytes, which resolves to %lu units\n", _numbytes, numunits);
  if (has_initialized == 0) {
     mymalloc_init();
  }

  struct mem_control_block *curr_free_block = free_list_start;
  int new_used_block_tot_size = numunits + 1;
  while(curr_free_block) {
    if (curr_free_block->size >= new_used_block_tot_size) {
      struct mem_control_block *curr_block_end_addr = curr_free_block + 1 + curr_free_block->size;
      struct mem_control_block *new_used_block = (struct mem_control_block *)(curr_block_end_addr - new_used_block_tot_size);
      new_used_block->size = numunits;
...
void myfree(void *firstbyte) {
  ...
  // merge left
  if (prev != NULL && prev + prev->size + 1 == firstblock) {
    prev->next = firstblock->next;
    prev->size += firstblock->size + 1;
  }
  // merge right
  if (firstblock->next != NULL && firstblock + firstblock->size + 1 == firstblock->next) {
    firstblock->size += firstblock->next->size + 1;
